...
int main() {               // The starting point where the program begins execution
    int a, b, result;      // Declare three integer variables to hold user inputs and the final value
    ...
    // Even though the name is 'addTwo', we are passing 'a' and 'b' to get their difference
    result = addTwo(a, b);               

    // Displaying output
    // This line prints the numbers entered and the result calculated by the function
    cout << "The result of the operation on " << a << " and " << b << " is: " << result << endl;

    return 0;              // Tell the operating system that the program finished without errors
}

// Function Definition: This is the logic requested in Modification 1
int addTwo(int x, int y) { // Receives the two integers from main() into local variables 'x' and 'y'
    return (x - y);        // TASK 9 MOD 1: The '+' is changed to '-' to perform subtraction
}